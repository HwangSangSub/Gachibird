...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr class="<?=$bg?>" title="<?=$row['taxi_Memo']?>">
        ...
        <td headers="mb_list_idx"><a href="<?=UDEV_DIR?>/taxiSharing/taxiSharingReg.php?mode=mod&idx=<?=$row['taxi_SIdx']?>&<?=$qstr?>&page=<?=$page?>" ><?=$row['taxi_SIdx']?></a></td>
        <td headers="mb_list_id"><?=$row['taxi_RMemId']?> (<?=$memNickNm?>) </td>
        <td headers="mb_list_id" ><a href="<?=UDEV_DIR?>/order/orderList.php?findType=taxi_OrdNo&findword=<?=$row['taxi_OrdNo']?>"><?=$row['taxi_OrdNo']?></a></td>
