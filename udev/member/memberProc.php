...
$memQuery = "SELECT idx /*, mem_ImgFile */ FROM member WHERE mem_id = :mem_Id AND disply = 'N'";
...
if ($mode == "reg") {
	...
	//회원 기본 수정
	$upQquery = "UPDATE member SET /* mem_NickNm = :mem_NickNm,  */ mem_pwd = :mem_Pwd /*, mem_lv = :mem_Lv, tel = :mem_Tel, birth = :mem_Birth, mem_ImgFile = :mem_ImgFile */ WHERE idx = :idx LIMIT 1";
	...
	//회원 기타 정보 수정
	$upQquery2 = "UPDATE member_info SET tel = :mem_Tel, birth = :mem_Birth, sex = :mem_Sex /*, mem_Seat = :mem_Seat, mem_Memo = :mem_Memo */ WHERE mem_idx = :idx LIMIT 1";
	$upStmt2 = $DB_con->prepare($upQquery2);
	$upStmt2->bindParam(":mem_Tel", $mem_Tel);
	$upStmt2->bindParam(":mem_Birth", $mem_Birth);
	...
	foreach ($array as $k => $v) {
		...
		//회원 아이디 검색
		$chkQuery = "";
		$chkQuery = " SELECT mem_id /*, mem_ImgFile */ FROM member WHERE idx = :idx ";
		//echo $chkQuery."<BR>";
		//exit;
		...
		if ($num == "0" && $cntNum == "0") { // 매칭신청 진행중인 회원이 없을 경우는 탈퇴처리
			...
			$b_Disply = "Y"; //탈퇴
			$reg_Date = TIME_YMDHIS;		   //탈퇴일


			//회원 이미지 삭제
			// @unlink($mb_dir . '/' . $memImgFile);

			$upQquery = "UPDATE member SET disply = :b_Disply, leave_date = :leaved_Date  WHERE idx = :idx LIMIT 1";
			$upStmt = $DB_con->prepare($upQquery);
			// $upStmt->bindparam(":mem_ImgFile", $mem_ImgFile);
			...
			$upQquery2 = "UPDATE member_info SET tel = :mem_Tel, birth = :mem_Birth,/* sex = :mem_Sex, mem_ImgFile = :mem_ImgFile, mem_Haddr = :mem_Haddr, mem_Oaddr = :mem_Oaddr,*/ mem_point = :mem_Point /*mem_MatCnt = :mem_MatCnt,*/ WHERE mem_idx = :idx LIMIT 1";
			$upStmt2 = $DB_con->prepare($upQquery2);
			$upStmt2->bindparam(":mem_Tel", $mem_Tel);
			$upStmt2->bindparam(":mem_Birth", $mem_Birth);
			// $upStmt2->bindparam(":mem_Sex", $mem_Sex);
			$upStmt2->bindparam(":mem_Point", $mem_Point);
			$upStmt2->bindParam(":idx", $chkIdx);
			$upStmt2->execute();
