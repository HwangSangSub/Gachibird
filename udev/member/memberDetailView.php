...
$viewQuery = "SELECT * 
	FROM member AS A 
		INNER JOIN member_info AS B ON A.idx = B.mem_idx 
	WHERE A.idx = " . $idx . " "; //LEFT OUTER JOIN TB_MEMBER_PHOTO AS C ON A.idx = C.mem_Idx
...
$mem_Birth = $row['birth'];						// 생년월일
// $mem_Email = $row['mem_Email'];						// 이메일
$mem_SnsChk = $row['mem_sns'];					// sns 로그인체크 
$mem_Sex = $row['sex'];							// 성별 0: 남자, 1:여자
$mem_Tel = $row['tel'];							// 연락처
