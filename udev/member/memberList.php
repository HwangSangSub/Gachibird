...
$sql_search = " WHERE A.mem_lv NOT IN ('0', '1', '2') AND A.disply = 'N' ";
...
if ($findOs != "") {
	$sql_search .= " AND B.mem_os = :mem_os";
}
...
if ($findword != "") {
	if ($findType == "mem_Id") {
		...
		$sql_search .= " AND B.tel LIKE :findword ";
	} else if ($findType == "mem_Nm") {
		$sql_search .= " AND B.name LIKE :findword ";
	}
}
...
//전체 카운트
$cntQuery = "";
$cntQuery = "SELECT COUNT(A.mem_id)  AS cntRow FROM member A INNER JOIN member_info B ON B.mem_idx = A.idx  {$sql_search}  ";
...
//목록
$query = "SELECT A.idx, A.mem_id,/* A.mem_NickNm,*/ B.cert_bit, B.tel, A.reg_date, B.mem_os, A.disply /*, A.mem_Code, B.mem_Point, B.mem_MatCnt,*/ ";
$query .= " FROM member A "; 
$query .= " INNER JOIN member_info B ON B.mem_idx = A.idx  {$sql_search} {$sql_order} LIMIT  {$from_record}, {$rows} ";
// echo $query."<BR>";
// exit;
...
//탈퇴회원수
$mcntQuery = "SELECT COUNT(idx) AS mCnt FROM member  WHERE disply = 'Y' ";
$mcntStmt = $DB_con->prepare($mcntQuery);
$mcntStmt->execute();
$mcRow = $mcntStmt->fetch(PDO::FETCH_ASSOC);
$leave_count = $mcRow['mCnt'];

$qstr = "fr_date=" . urlencode($fr_date) . "&amp;to_date=" . urlencode($to_date) . "&amp;findType=" . urlencode($findType) . "&amp;rows=" . urlencode($rows) . "&amp;findword=" . urlencode($findword);
						...
						<tbody>
							...
							if ($numCnt > 0) {
								...
								while ($row = $stmt->fetch()) {
									...
									//회원 정보 
									$mInfoQuery = "";
									$mInfoQuery = "SELECT B.sex, B.mem_sns, A.login_date from member A join member_info B on B.mem_idx = A.idx  WHERE A.idx = :idx  LIMIT 1";
									$meInfoStmt = $DB_con->prepare($mInfoQuery);
									$meInfoStmt->bindparam(":idx", $memIdx);
									$meInfoStmt->execute();
									...
									if ($infoNum < 1) { //아닐경우
										...
										while ($ifnoRow = $meInfoStmt->fetch(PDO::FETCH_ASSOC)) {
											$mem_Sex = trim($ifnoRow['sex']);							// 성별 (0:남자 , 1:여자)
											// $mem_Seat = trim($ifnoRow['mem_Seat']);						// 좌석 (0:앞자리 , 1:뒷자리)	
											$mem_SnsChk = trim($ifnoRow['mem_sns']);				// sns구분 ( kakao, google )
							...
							?>
									...
									<tr class="<?= $bg ?>">
										...
										<td headers="mb_list_open" class="td_name td_mng_s"><?= $row['tel'] ?></td>
										<!-- <td headers="mb_list_open" class="td_name td_mng_s"><?= $memCard ?></td> -->
										<td headers="mb_list_open" class="td_mbstat td_mng_s"><?= $mem_Sex ?></td>
