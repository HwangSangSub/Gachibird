						...
						<tbody>
							...
							?>
									...
									<tr class="<?= $bg ?>" title="<?= $taxi_Memo ?>">
										...
										<td headers="mb_list_sidx" style="width:5%;"><?= ($taxi_SIdx == "-" ? '' : '<a href="' . UDEV_DIR . '/taxiSharing/taxiSharingReg.php?mode=mod&idx=' . $taxi_SIdx . '&' . $qstr . '&page=' . $page . '" >') ?><?= $taxi_SIdx ?><?= ($taxi_SIdx == "-" ? '' : '</a>') ?></td>
										<? if ($pointchk == "0") { ?> <!--일반 주문-->
											<td headers="mb_list_order"><a href="<?= UDEV_DIR ?>/order/orderList.php?findType=taxi_OrdNo&findword=<?= $taxi_OrdNo ?>"><?= $taxi_OrdNo ?></a></td>
										<? } else if ($pointchk == "1") { ?> <!--이벤트포인트적립-->
											<td headers="mb_list_order"><?= $taxi_OrdNo ?></td>
										<? } else if ($pointchk == "2") { ?> <!--환전-->
											<td headers="mb_list_order"><a href="<?= UDEV_DIR ?>/account/pointExcList.php?findType=idx&findword=<?= $taxi_OrdNo ?>"><?= $taxi_OrdNo ?></a></td>
										<? } else { ?> <!--그외-->
											<td headers="mb_list_order"><?= $taxi_OrdNo ?></td>
										<? } ?>
