...
if ($mode == "mod") {
	...
	$query = "
			SELECT 
				...
				info.birth
				FROM member
				INNER JOIN member_info AS info ON member.idx = info.mem_idx
				WHERE member.idx = :idx AND member.disply = 'N' ";
				/* 
				member.mem_NickNm,
				member.mem_Code, 
				member.mem_CharBit, 
	...
	if ($num < 1) { //아닐경우
		...
		while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
			...
			$mem_Tel = trim($row['tel']);
			$mem_Birth = trim($row['birth']);
    ...
    */
			...
			//회원 정보
			$mInfoQuery = "";
			$mInfoQuery = "SELECT sex, /*mem_Seat, mem_Email, */ mem_sns from member_info WHERE mem_Idx = :mem_Idx LIMIT 1";
			...
			//회원 기타 정보
			$mEtcQuery = "";
			$mEtcQuery = "SELECT mem_point /* , mem_MatCnt, mem_McCnt */ FROM member_info WHERE mem_idx = :mem_Idx LIMIT 1";
						...
						<tbody>
							...
							<tr>
								...
								<td scope="row" colspan="3">
									<input type="radio" name="mem_Sex" value="M" id="mem_Sex" <?= ($mem_Sex == "M" || !$mem_Sex) ? "checked" : ""; ?>>
									<label for="mem_Sex">남자</label>
									<input type="radio" name="mem_Sex" value="F" id="mem_Sex" <?= ($mem_Sex == "F") ? "checked" : ""; ?>>
