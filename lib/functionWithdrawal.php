...
function memImgDel($mem_Idx)
{
    ...
    $chkMQuery = " SELECT mem_ImgFile FROM member_info WHERE idx = :mem_idx ";
    //echo $chkMQuery."<BR>";
    //exit;
...
function memUPDate($mem_Idx)
{
    ...
    //회원 기타 조회 후 임시 보관
    $memQuery  = "";
    $memQuery  = " SELECT A.idx, A.mem_lv, B.nicknm, B.name, B.birth, B.tel, B.cert_id FROM member A ";
    $memQuery .= " LEFT OUTER JOIN member_info B ";
    $memQuery .= " ON B.mem_idx = A.idx ";
    $memQuery .= " WHERE B.mem_idx = A.idx ";
    $memQuery .= "AND A.idx = :mem_Idx AND A.disply = 'N' ";
    ...
    if ($memNum < 1) { //아닐경우
        ...
        $mem_Lv = 0;  //회원레벨
        $b_Disply = "Y";  //탈퇴
        $reg_Date = TIME_YMDHIS;     //등록일
        $mem_SecedeEtc = TIME_YMDHIS . " 본인이 탈퇴 처리함.";

        while ($memRow = $memStmt->fetch(PDO::FETCH_ASSOC)) {
            $memTel = trim($memRow['tel']);          // 연락처
            $memLv = trim($memRow['mem_lv']);          // 등급
            $memNm = trim($memRow['name']);          // 등급
            // $memNickNm = trim($memRow['mem_NickNm']);          // 등급
            $memCertId = trim($memRow['cert_id']);          // 등급
