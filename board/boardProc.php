...
if ($mode == "reg" || $mode == "rep") {  //등록, 답변
	...
	$bchkQuery = "";
	$bchkQuery = "SELECT MAX(idx) AS idx FROM board WHERE type = :b_Idx LIMIT 1";
...
*/
	...
	$insQuery = "INSERT INTO board (type, mem_idx, mem_name, title, content, r_content, reg_date, read_cnt, is_notice, disply) VALUES (:b_Idx, :b_MemIdx, :b_Name, :b_Title, :b_Content, :b_RContent, :reg_Date, 0, :b_Not, 'Y')";
	//echo $insQuery."<BR>";
	//exit;
	...
	if ($stmt->rowCount() > 0) { //삽입 성공
		$preUrl = "../board/boardList.php?type=" . $b_Idx;
		$message = "reg";
	...
	$upQquery = "UPDATE board SET title = :title, content = :content WHERE idx = :idx LIMIT 1";
	...
	// $upStmt->bindparam(":is_notice", $b_Not);
	$upStmt->bindparam(":title", $b_Title);
	// $upStmt->bindparam(":mem_name", $b_Name);
	$upStmt->bindparam(":content", $b_Content);
	...
	foreach ($array as $k => $v) {
		...
		//첨부파일폴더
		$foldQuery = "";
		$foldQuery = "SELECT upload FROM board_info  WHERE idx = :b_Idx LIMIT 1 ";
		$foldStmt = $DB_con->prepare($foldQuery);
		$foldStmt->bindparam(":b_Idx", $b_Idx);
		$foldStmt->execute();
		$foldNum = $foldStmt->rowCount();
		...
		if ($foldNum < 1) { //아닐경우
			...
			while ($foldRow = $foldStmt->fetch(PDO::FETCH_ASSOC)) {
				$chkUpload = trim($foldRow['upload']);					//첨부파일 폴더
			}
		}

		//첨부파일
		$bFileQuery = "";
		$bFileQuery = "  SELECT idx, board_idx, name, original_name, size FROM board_file ";
		$bFileQuery .= "  WHERE board_idx = :b_Idx LIMIT 1 ";
		$bFileStmt = $DB_con->prepare($bFileQuery);
		$bFileStmt->bindparam(":b_Idx", $check);
		...
		if ($bFileNum < 1) { //아닐경우
			...
			while ($bFileRow = $bFileStmt->fetch(PDO::FETCH_ASSOC)) {
				...
				$delFileQuery = "DELETE FROM TB_BOARD_FILE WHERE board_idx = :b_Idx LIMIT 1";
				$delFileStmt = $DB_con->prepare($delFileQuery);
				$delFileStmt->bindParam("b_Idx", $check);
				$delFileStmt->execute();
			}
		}

		//댓글 삭제
		$delComQuery = "DELETE FROM board_comment WHERE board_idx = board_idx LIMIT 1";
		$delComStmt = $DB_con->prepare($delComQuery);
		$delComStmt->bindParam("b_Idx", $check);
		$delComStmt->execute();


		//게시물 삭제
		$delQuery = "DELETE FROM board WHERE idx = :b_Idx LIMIT 1";
		$delStmt = $DB_con->prepare($delQuery);
		$delStmt->bindParam("b_Idx", $check);
		$delStmt->execute();
