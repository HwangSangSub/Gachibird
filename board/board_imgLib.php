<?
	...
	//게시판 환경설정
	$bquery = "";
	$bquery = "SELECT upload FROM  board_info WHERE idx = :b_Idx LIMIT 1";
	...
	if($bNum < 1)  { //아닐경우
		...
		while($bsRow=$bStmt->fetch(PDO::FETCH_ASSOC)) {
			$b_Upload = trim($bsRow['upload']);			
			$b_UploadFChk = "jpg,jpeg,gif,bmp,png,psd,swf,tar,gz,tgz,alz,zip,rar,ace,arj,tif,doc,docx,xls,xlsx,hwp,ppt,pptx,pdf,dwg";			// trim($bsRow['b_UploadFChk'])
		}
	}
		...
		if(is_array($_FILES['files']['name'])){
							...
							if(!is_dir($chkUpload2)){
								...
								if ($mode == "mod") { 
									...
									// $chkFQuery = " SELECT idx, b_Idx, b_NIdx, b_FIdx, b_FName, b_FSize FROM TB_BOARD_FILE  WHERE b_Idx = :b_Idx AND b_NIdx = :b_NIdx AND b_FIdx = :b_FIdx ";
									 $chkFQuery = " SELECT idx, board_idx, name, original_name, size FROM board_file  WHERE board_idx = :b_Idx ";
									 ...
									 if ($chkFNum < 1)  {
										...
										while($chkFRow=$chkFStmt->fetch(PDO::FETCH_ASSOC)) {
											...
											$dFName = trim($chkFRow['name']);		
											$dFIdx = trim($chkFRow['idx']);		
			...
			$chkimg = $FileName;	
									...
									if ($mode == "reg") {
										...
										//파일첨부 저장
										$finsQuery = "INSERT INTO board_file (board_idx, name, original_name, size, reg_date, mod_date) VALUES (:b_Idx, :b_FName, :b_OFName, :b_FSize, :reg_date, :mod_date)";
										...
										$fstmt->bindParam(":reg_date", $regDate);
										$fstmt->bindParam(":mod_date", $regDate);
										$fstmt->execute();
										$DB_con->lastInsertId();
										...
										if ($chkFNum < 1)  { 
											...
											//파일첨부 저장
											$fMinsQuery = "INSERT INTO board_file (board_idx, name, original_name, size, reg_date, mod_date) VALUES (:b_Idx, :b_FName, :b_OFName, :b_FSize, :reg_date, :mod_date)";
											...
											$fMstmt->bindParam(":reg_date", $regDate);
											$fMstmt->bindParam(":mod_date", $regDate);
											$fMstmt->execute();
											$DB_con->lastInsertId();
											...
											while($chkFRow=$fMstmt->fetch(PDO::FETCH_ASSOC)) {
												...
												$upfileQuery = "UPDATE board_file SET name = :b_FName, original_name = :b_OFName, size = :b_FSize, mod_date = :mod_date WHERE idx = :idx LIMIT 1";
												...
												$upStmt->bindparam(":mod_date",$regDate);
